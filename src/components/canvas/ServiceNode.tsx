...
export const ServiceNode = memo(({ data, selected }: NodeProps) => {
  ...
  return (
    <div
      className={cn(
        "w-[320px] rounded-xl bg-[#161616] border transition-colors",
        selected ? "border-emerald-500 shadow-[0_0_0_1px_rgba(16,185,129,1)]" : "border-[#2a2a2a] hover:border-white/20"
      )}
    >
      <Handle type="target" position={Position.Top} className="w-3 h-3 border-2 border-[#161616] bg-emerald-500" />
      ...
      {/* Metrics Grid */}
      <div className="grid grid-cols-4 divide-x divide-white/5 border-b border-white/5 bg-[#111]">
      ...
      {/* Tabs */}
      <div className="flex px-4 pt-3 pb-2 gap-4 text-xs font-medium border-b border-white/5">
      ...
      {/* Footer Status */}
      <div className="flex items-center justify-between p-3 bg-black/40 rounded-b-xl border-t border-white/5">
        <div className={cn(
      ...
      <Handle type="source" position={Position.Bottom} className="w-3 h-3 border-2 border-[#161616] bg-emerald-500" />
    </div>
  )
})
